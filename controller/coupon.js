...
const getCoupon = asyncHandler(async (req, res, next) => {
    ...
    result &&
        res.status(200).json({ message: "Coupon Found Successfully", result });
});

const updateCoupon = asyncHandler(async (req, res, next) => {
    const { id } = req.params;
    const review = await couponModel.findOneAndUpdate(id, req.body, {
        new: true,
    });
    !review &&
        next(
            new Error(
                "coupon Not Found Or You are not authorized to Update this review",
                { cause: 404 }
            )
        );
